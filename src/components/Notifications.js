...
//             Tutaj będą pojawiać się wszystkie ważne informacje systemowe.
//           </Typography>
//         </Paper>
//       ) : (
...
//               Wiadomość zostanie dostarczona do wszystkich aktywnych użytkowników w czasie rzeczywistym.
//             </Alert>
//           </Box>
//         </DialogContent>
...
function Notifications() {
  ...
  useEffect(() => {
    ...
    // Правильный запрос - ищем уведомления для текущего пользователя по его UID
    const q = query(
      notificationsRef,
      where("userId", "==", currentUser.uid),
      orderBy("createdAt", "desc"),
    ...
    try {
      ...
      for (const userDoc of usersSnapshot.docs) {
        try {
          // Отправляем уведомление каждому пользователю
          await addDoc(collection(db, "notifications"), {
            userId: userDoc.id, // Важно: используем ID пользователя из коллекции users
            title: "📢 Wiadomość od administratora",
            message: adminMessage,
            type: "admin_message",
            priority: adminPriority,
  ...
  const getIcon = (notification) => {
    ...
    if (notification.type === "inactive_school") {
      return <SchoolIcon />;
    }
    if (notification.type === "upcoming_action") {
      return <ScheduleIcon />;
  ...
  return (
    <Box>
      <Paper
        ...
        elevation={0}
        sx={{ p: 0, mb: 3, border: "1px solid #e0e0e0" }}
      >
        <Box
          sx={{
            display: "flex",
      ...
      >
        ...
        <DialogContent dividers sx={{ p: 3 }}>
          <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
                ...
                {[
                  ...
                  <Grid item xs={4} key={p.value}>
                    <Button
                      fullWidth
                      variant={
                        adminPriority === p.value ? "contained" : "outlined"
                      }
                      color={p.color}
                      onClick={() => setAdminPriority(p.value)}
                      startIcon={p.icon}
            ...
            <Alert severity="info" variant="outlined" sx={{ borderRadius: 2 }}>
              Wiadomość zostanie dostarczona do wszystkich aktywnych
              użytkowników w czasie rzeczywistym.
            </Alert>
          </Box>
        </DialogContent>
        <DialogActions sx={{ p: 3, gap: 1 }}>
          <Button onClick={() => setShowAdminMessage(false)} color="inherit">
            Anuluj
          </Button>
          <Button
            onClick={sendAdminMessage}
